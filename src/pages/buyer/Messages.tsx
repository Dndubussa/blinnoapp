...
export default function BuyerMessages() {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Optimized query using joins instead of N+1
      const { data: convos, error } = await supabase
        ...
        .select(
          ...
          *,
          ...
          messages!inner(content, created_at, is_read, receiver_id)
          `
        )
      ...
      const enrichedConvos = (convos || []).map((conv: any) => {
        const otherUserId = conv.buyer_id === user.id ? conv.seller_id : conv.buyer_id;
        const otherUserProfile = conv.buyer_id === user.id ? conv.seller_profile : conv.buyer_profile;
        
        // Get last message and unread count from the messages array
        ...
        return {
          ...conv,
          other_user: otherUserProfile || { id: otherUserId, full_name: "Unknown", avatar_url: null },
          last_message: lastMsg?.content,
          unread_count: unreadCount,
          messages: undefined, // Remove to save memory
        };
      });
